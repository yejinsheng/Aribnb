<template>
    <div class="container-fiuld">
        ...
        <div class="my_nav nav nav-justified" style="height:50px;background:white">
            <a href="#">详情</a>·
            <a href="#">评价</a>·
            <a href="#">可订日期</a>·
            <a href="#">位置</a>·
        ...
        <div class="row">
            <div class="col-lg-5 offset-lg-2">
                ...
                <div class="pt-5">
                    ...
                    <div class="envaulte">
                        <textarea id="textarea" cols="30" rows="10" placeholder="发表评论" v-model="story"></textarea>
                        <input type="button" value="发表评论" class="pinglun" @click="comments">
                    </div>
                    <hr style="width:75%">
                    <div class="show_pinglun w-100" v-for="p in output.data" :key="p.id">
                       <hr style="width:75%">
                       <div >
                    ...
                    <div class="fenye">
                        <ul class="item">
                            <li @click="fening(-2)">上一页</li>
                            <li v-for="item in output.count" :key="item" @click="fening(item-1)" :class="item-1 ==pageIndex?'active':''">{{item}}</li>
                            <li @click="fening(-1)"><span >下一页</span></li>
                        </ul>
                    </div>
                </div>
                <hr style="width:75%">
...
export default {
  ...
  methods: {
    comments() {
      this.axios.post("/app/pinglun/comment",Qs.stringify({story: this.story})).then(res => {
    ...
    fen(){
        this.axios.get("/app/pinglun/comment_fenye?pageIndex=" + this.pageIndex).then(res => {this.output = res.data;});
    },
    fening(item){
        if(item==-2){
            if(this.pageIndex>0){
            ...
            if(this.pageIndex+1<this.output.count){
                this.pageIndex=this.pageIndex+1;
                this.fen();
            }
        }else if(item != this.pageIndex){
            this.pageIndex=item;
            this.fen();
        }
    }
